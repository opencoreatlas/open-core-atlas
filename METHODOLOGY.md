...
- **Add `"source-available-tiered"` to `commercial_model.type` enum.** Companies that no longer have an OSI-open OSS layer but retain (a) source-available code, (b) free usage thresholds (revenue caps, seat caps, or non-production use carve-outs), (c) paid self-hosted licensing, and (d) hosted cloud offerings. Current enum (`pure-open-core`, `hosted-first`, `hybrid`, `services-support`, `saas-with-oss-marketing`) all assume the existence of an OSS layer in their definitions. The post-2023/2024 wave of license migrations created a population of companies that strictly fit none of these. CockroachDB post-2024 CSL is the canonical example; candidate fits also include Redis post-license change, Elastic / SSPL-style models, HashiCorp post-BSL, Sentry post-FSL transition. First flagged during Phase 3 of the CockroachDB walkthrough (forced 'hybrid' as least-bad fit; documented in record's `metadata.notes`). Targeted for schema v1.1.
