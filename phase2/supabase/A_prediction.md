# Supabase — Phase 2A Prediction

Status: PREDICTION_ONLY
Evidence opened: NO
Prediction author: Pablo
Created: 2026-05-31

## 0. Anchor data

Founded year:
Source for founded year (if checked):

(Note: founded_year is needed to make the cohort prediction in §2 non-trivial. If you already know it, write it. If you would need to look it up, decide whether that counts as "opening evidence" — see §7.)

## 1. LP Role Prediction

Prediction: `absent | defender | detonator | unknown`

Rationale:
-

## 2. Cohort Prediction

Bins (from PHASE2_PROTOCOL.md v0.1):
- `historical-cohort`: founded_year ≤ 2012
- `borderline`: 2013 ≤ founded_year ≤ 2017
- `post-pattern`: founded_year ≥ 2018
- `unknown`: founded_year not known without opening evidence

Prediction: `historical-cohort | borderline | post-pattern | unknown`

Rationale:
-

## 3. Expected Classification

Trajectory: `native | pivot | evolving`
Distribution Posture: `cloud-only | cloud-first | dual-path | self-host-first`
Primary Strategy: `LP | GG | TfI | CP | DND`
Secondary Strategies:

Rationale:
-

## 4. Expected Taxonomy Stress

Prediction: `none | mild ambiguity | severe ambiguity | unclassifiable`

Rationale:
-

## 5. Surprising Result

A result that would surprise this prediction:
-

## 6. Fourth Scenario

A scenario not contemplated by the taxonomy v0 that would itself count as evidence against v0:
-

## 7. Locked Notes

This file must be committed before Phase 2B begins.

No evidence from Supabase docs, pricing pages, GitHub repos, blog posts, recent funding news, or prior Atlas working notes may be opened before this file is committed.

Prior general knowledge (the kind a careful operator already has from being in the ecosystem) is acceptable as a human prior. The line is between "what I already know" and "what I would have to look up". The second is evidence; opening it now invalidates the experiment.

`unknown` is a valid prediction in any field when the answer would require opening new evidence. Guessing is worse than missing. The experiment measures the real prior, not the ability to guess.
